...
namespace XNode
{
    ...
    public class NodePort
    {
        ...
        public void Connect(NodePort port)
        {
            ...
            if (port == null) { UnityEngine.Debug.LogWarning("Cannot connect to null port"); return; }
            if (port == this) { UnityEngine.Debug.LogWarning("Cannot connect port to self."); return; }
            if (IsConnectedTo(port)) { UnityEngine.Debug.LogWarning("Port already connected. "); return; }
            if (direction == port.direction && !port.IsAttachedToGraphHolder) { UnityEngine.Debug.LogWarning("Cannot connect two " + (direction == IO.Input ? "input" : "output") + " connections"); return; }
